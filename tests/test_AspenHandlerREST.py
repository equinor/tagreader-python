...
@pytest.fixture()
def AspenHandler():
    h = AspenHandlerWeb(server="servername")
    yield h
...
def test_generate_search_query():
    ...
    assert AspenHandlerWeb.generate_search_query("ATCAI", server="servername") == {
        "datasource": "servername",
        "tag": "ATCAI",
        "max": 100,
        "getTrendable": 0,
    }
    assert AspenHandlerWeb.generate_search_query("ATC*", server="servername") == {
        "datasource": "servername",
        "tag": "ATC*",
        "max": 100,
        "getTrendable": 0,
    }
    assert AspenHandlerWeb.generate_search_query(tag="ATCAI", server="servername") == {
        "datasource": "servername",
        "tag": "ATCAI",
        "max": 100,
        "getTrendable": 0,
...
def test_generate_description_query(AspenHandler):
    assert (
        AspenHandler.generate_get_description_query("ATCAI") == (
            '<Q allQuotes="1" attributeData="1"><Tag><N><![CDATA[ATCAI]]></N><T>0</T>'
            "<G><![CDATA[ATCAI]]></G><D><![CDATA[servername]]></D><AL><A>DSCR</A>"
            "<VS>0</VS></AL></Tag></Q>"
        )
    )
...
def test_generate_unit_query(AspenHandler):
    assert (
        AspenHandler.generate_get_unit_query("ATCAI") == (
            '<Q allQuotes="1" attributeData="1"><Tag><N><![CDATA[ATCAI]]></N><T>0</T>'
            "<G><![CDATA[ATCAI]]></G><D><![CDATA[servername]]></D><AL><A>Units</A>"
