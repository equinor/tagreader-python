...
class AspenHandlerODBC:
    ...
    @staticmethod
    def generate_read_query(tag, mapping, start_time, stop_time, sample_time, read_type):
        ...
        request_num = {ReaderType.SAMPLED: 4,
        ...
        from_column = {ReaderType.SAMPLED: 'history',
        ...
        ts_actual = {ReaderType.MIN: 'ts_middle',
    ...
    def read_tag(self, tag, start_time, stop_time, sample_time, read_type, metadata):
        ...
        query = self.generate_read_query(cleantag, map_historyvalue, start_time, stop_time, sample_time, read_type)
        #logging.debug(f'Executing SQL query {query!r}')
        df = pd.read_sql(query, self.conn, index_col='time',
...
class PIHandlerODBC:
    ...
    def generate_connection_string(host, port, das_server):
        return (f'DRIVER={{PI ODBC Driver}};Server={das_server};Trusted_Connection=Yes;Command Timeout=1800;'
    ...
    def generate_read_query(tag, start_time, stop_time, sample_time, read_type, metadata=None):
        ...
        source = {ReaderType.INT: '[piarchive]..[piinterp2]',
                  ...
                  ReaderType.SHAPEPRESERVING: '[piarchive]..[piplot]'}.get(read_type, '[piarchive]..[picomp2]')
        ...
        query.extend([f"AS value, __utctime AS timestamp FROM {source} WHERE tag='{tag}'"])
        ...
        if ReaderType.GOOD == read_type:
            ...
            query.extend([f"AND (intervalcount = {int((stop_time-start_time).seconds/sample_time)})"])
    ...
    def connect(self):
        connection_string = self.generate_connection_string(self.host, self.port, self._das_server)
        # The default autocommit=False is not supported by PI odbc driver.
        self.conn = pyodbc.connect(connection_string, autocommit=True)
        self.cursor = self.conn.cursor()
